...
- 「絶対サーバに届いてほしいデータは confirm にする」のは決定的な思い違いである。
「データは unconfirm で複数回送って１回でも届けばよし」
「たまに小容量の confirmで死活監視（ついでに unconfirm が何回届いたかサーバに聞いてみる）」とするのが正しい。有線LANとは棲み方が違う。
- 一瞬で済む送信よりも数秒間ものあいだ耳を済ませねばならない受信のほうが、電力消費がはるかに激しい。confirm は使えば使うだけ自滅する。
...
- LoRaWAN の使う電波帯は、そもそもアマチュア無線でもつかわれる超短波 33cm波長域の一種だ。成層圏で反射も吸収もされないがゆえ月や火星とでも通信できる。一方で国と地域によってさまざまな用途でフリーダムな使われ方をしているので混信は回避できない。LoRaWAN はそのなかへほそいキリをえぐりこむかのような手段をとるワイルドな規格である。届くときは届くが、届かないときは届かない。
