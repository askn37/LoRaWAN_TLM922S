...
なお TLM922S は LoRaWAN ClassB仕様に準拠してはいるが、本ライブラリは ClassAのみの対応となる。

# LoRaWANについて

LoRaWAN は半二重ハンドシェイク通信方式をとる。
まず事前に joinコマンドで LoRaWANゲートウェイとのセッション開始の宣言をおこなったあと、
...
別のチャンネルを使用することで全二重通信を行っているのとは対照的である。
LoRaWANの通信方式はトランシーバーや、外惑星探査機のそれに近い。
さらにアップ・ダウン両パケットとも、相手に必ず届くことを保証しておらず
これはインターネットにおけるUDP方式と同等のものだ。
さらに電波帯域の利用も QoSに当たるものがなく、ベストエフォートのみとなる。

LoRaWAN はあえてこの方式を取ることと、
ペイロードデータ量を小さく制限し、
かつエラー訂正を強化しつつビットレートをごく低く抑えることで、
乾電池駆動の低送信電力でありながら直線見通し次第で 100kmにも達する超遠距離通信を可能にしている。

一方で片方向限定の多対1通信には得手で、
ひとつの基地局（ゲートウェイ）で数千から数万デバイスのアップリンクを処理することも難しくはない。
つまりわずかなデータを間欠送信するだけの廉価なセンサーデバイスを大量にばらまき、
...
コマンドが実行された時点で通信が切れてしまうため、このメソッドには返値がない。
...
```c
if (LoRaWAN.save()) Serial.println("mod save ok");
```
