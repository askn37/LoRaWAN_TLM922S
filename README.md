...
- SenseWay Inc. [ADB922S](https://store.senseway.net/items/15182297)
...
安定して運用するには 150mA以上の LDO と、33uF 以上の平滑コンデンサを備えたほうが良い。
（もし超省電力待機機能 Deep Sleep を使うなら、電源品質への要求はより厳しいので注意すること）
...
### bool sleep (uint16\_t SECONDS, bool DEEPSLEEP = false)

TLM922Sを Standby Sleep モードにする。
スリープ時間には 1～65535秒と、0==無制限が指定できる。
不正な値には偽を返し、PS\_INVALID となる。

第2引数に真を与えると Deep Sleep モードが使われる。詳しくは後述。
...
本コマンドの既定値は Standby Sleep モードであるが、これの実行中消費電力は約 330uA 以下である。
Deep Sleep モードを選択すると実行中消費電力は約 2uA 以下にまで削減できるが、
以下の制約に注意しなければならない。

- Deep Sleep 使用時は、データ線に 3.8V を超える電圧が印加されないようにすること。
つまり通常の 5V信号系 Arduino 互換機に直接積み重ねた状態では、使用してはならない。
Deep Sleep 中はデータ線に印加された電圧が制御されることなく 3.3V電源線へ逆流する。
これは他に接続している 3.3V センサー等を破壊するかもしれない。
- Deep Sleep からの復帰時に、非常に大きな突入電流とサージノイズが流れうる。
これは Deep Sleep 継続時間により時定数で増加するが、
100〜150mA かそれ以上に達し、電源容量によっては 1V以上の電圧降下を伴うかもしれない。
電源線には余裕のあるコンデンサや保安回路を接続しておくこと。
  - 電源には瞬間出力が 300mA に満たない（内部抵抗の大きな）一次電池は適さない。
300mA 容量以上の LDO や、600mAh 以上の LiPo 二次電池を使用すべきである。
なお近代的な LDO は様々な保安機能（サージ保護、過電流保護、突入電流緩和、etc）を備えているため、
LiPo 使用時でも可能なら挿入すること。
  - Standby Sleep 終了時の突入電流は、瞬間的には 60mA 程度である。
目安になるコンデンサ容量は 33〜47uF 程。リチウム系一次電池でも充分動作する。
  - Deep Sleep 解除を WAKEUP だけではなく /RESET 投入を先に行う制御回路も検討する。
この方法は電圧降下が激しい場合の異常動作を回避するのに役立つ。
ただし1回のリセット毎に大電力を消費するので
スリープ間隔が数時間以上でもなければ折り合わないだろう。

Sleep が正しく実行できない（十分安定していない）電源状態下では、
限度を超えた電圧降下によりモジュールリセットが発生する。
この予期しないリセットは、電子回路に不要なサージノイズを与え、ラッチアップを招くことがある。
そうなると電源線は常に 数mA〜数十mA の電流が流れ続け、電圧降下したままになって急速に電池を消耗するだろう。
この状態になったならば速やかに電源を切らなければならない。
...
- 0.1.4
  - ヘッダインクルードルールを修正。
  SoftwareSerial / MultiUART は明示的に、事前にインクルードしなければ
  有効化されないようにした。これによって未使用ライブラリのコンパイルが抑制される。
  - 全ファイルのインデントを修正。スペースとタブが混在していたので。
