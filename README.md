...
Region AS923 では日本の電波法規定により、各種パラメータが以下のように定まる。
工場出荷初期値は DR2である。
...
Dwell値（ドエルタイム＝休止時間挿入）は使用帯域によって制約が異なる。
Dwell=0 が選べるのは 923.4Mhz以下限定という規定があるため、
TLM922Sでは全38チャンネルで使用可能な Dwell=1 が標準値である。
（ファームウェアによっては、設定変更自体が不可）

Dwell=0 で DR4以上のペイロードサイズが頭打ちなのは、
ドエルタイムがないのでそれ以上の出力は電波法違反だからである。

Dwell=1 でDR0/DR1のペイロードサイズがゼロバイトなのは、
13byteのヘッダすらも発信制約時間内に収まらなくなることによる。
単純に Dwell=1 での DR0/DR1 の送信は電波法違反である。
TLM922S の場合は設定こそできるものの
txコマンドが PS_INVALID を返して発信拒否するようになっている。（受信は可能）

DR値を高めるほど送信出力が小さくなるが、
エンドノードからゲートウェイへの到達距離には DR2と DR5でほとんど差がない。
むしろ同じサイズのペイロードを送るなら、相対的に短時間で済む DR5のほうが成功率は高まる。
差が出るのはゲートウェイからエンドノードへむかうダウンリンクのほうで、
エンドノードの利得の低い小さなアンテナでは、ACK応答が取れない確率が高まる。

DR6は送信に同時に2チャンネル分の帯域を使うことでビットレートを倍速にする方式だが、
ヘッダを含めた最大パケット長が 255byteなのでペイロードサイズ自体は DR5と変わらない。
また PS_NOCHFREE応答が返る可能性も高まるため、屋外ではほとんど使えない。

瞬間最大ビットレートは以上を踏まえたうえで
電波発信時間の 10倍の沈黙時間を挟まなければならない規定があるため、
最大ペイロードでは DR5以下のすべてで約 4.5秒に1パケット送出が最大スループットとなる。
（DR6は約 2.2秒に1パケット送出可能なのでおよそ倍になる）
...
# LoRaWAN アプリケーション作成の心得

- LoRaWANは往復大量通信用途ではない。常に半二重であることを意識する。
- 「絶対サーバに届いてほしいデータは confirm にする」のは決定的に間違いである。
「データは unconfirm で複数回送って１回でも届けばよし」
「たまに小容量の confirmで死活監視（ついでに unconfirm が何回届いたかサーバに聞いてみる）」とするのが正しい。有線LANとは住む次元が違う。
- 一瞬で済む送信よりも数秒間ものあいだ耳を済ませねばならない受信のほうが、電力消費がはるかに激しい。confirm は使えば使うだけ自滅する罠である。
- データは早く小さいほどよく届く。1km超は当たり前。肉眼で直接見えてれば最小出力でも 4kmくらい本気で届く。サイズを半分にすれば成功率も倍になる。
- 目的と用途に応じてひとつのエンドノードでも DR値と confirm を適切に使い分ける。
