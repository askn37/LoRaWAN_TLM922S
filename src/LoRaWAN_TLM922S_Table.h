...
#define PS_DICT_MAX (193) 
#define CMD_DIC_LEN (178)
#define CMD_TABLE_LEN (25)

extern const PROGMEM uint32_t PS_DICT[];    // 772 bytes
...
typedef enum tlmps {
    ...
    , PS_PASSWORDERR    = 26    // >> Password Error
} tlmps_t;
...
typedef enum tlmex {
    ...
    , EX_LORA_SAVE          = 9     // lorawan save\r [ex_resetAfter]
    , EX_LORA_GET_DEVADDR   = 10    // lorawan get_devaddr\r [ex_hex]
    , EX_LORA_JOIN_OTAA     = 11    // lorawan join otaa\r []
    , EX_LORA_JOIN_ABP      = 12    // lorawan join abp\r []
    , EX_LORA_TX_UCNF       = 13    // lorawan tx ucnf <F> [ex_tx]
    , EX_LORA_TX_CNF        = 14    // lorawan tx cnf <F> [ex_tx]
    , EX_LORA_GET_DR        = 15    // lorawan get_dr\r [ex_dec F]
    , EX_LORA_GET_UPCNT     = 16    // lorawan get_upcnt\r [ex_dec T]
    , EX_LORA_GET_DWCNT     = 17    // lorawan get_downcnt\r [ex_dec T]
    , EX_LORA_GET_ADR       = 18    // lorawan get_adr\r [ex_onoff]
    , EX_LORA_SET_DR        = 19    // lorawan set_dr <F> []
    , EX_LORA_SET_ADR_OFF   = 20    // lorawan set_adr off\r []
    , EX_LORA_SET_ADR_ON    = 21    // lorawan set_adr on\r []
    , EX_LORA_SET_LINK      = 22    // lorawan set_linkchk\r []
    , EX_MOD_SET_BAUD       = 23    // mod set_baudrate <F> [ex_resetAfter]
